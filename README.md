...
- GitHub CLI・Git 運用関連 → `docs/github-notes.md`
...
```bash
git clone https://github.com/YusukeHarada/claude-settings.git ~/develop/claude-settings
```

### 2. CLAUDE.md をグローバル設定に反映

Claude Code は `~/.claude/CLAUDE.md` を全プロジェクトで自動参照する。symlink にしておくと `git pull` だけで最新が反映される。

```bash
mkdir -p ~/.claude
ln -sf ~/develop/claude-settings/CLAUDE.md ~/.claude/CLAUDE.md
```

### 3. 最新を取得する

```bash
cd ~/develop/claude-settings
