...
Claude Code が全プロジェクトで自動参照するガイドライン。このファイルは
`~/develop/claude-settings/CLAUDE.md` に置かれており、`~/.claude/CLAUDE.md`
から `@` インポートで読み込まれることで全プロジェクトに適用される。

言語設定・ワークフロー・コードスタイル・Git 運用・アーキテクチャ方針・
iOS Safari 対応など、プロジェクト横断で共通するルールをまとめている。
