...
body {
  overflow-x: clip; /* hidden ではなく clip（hidden だとピンチズームが無効になる） */
}
```
...
### Next.js 15 で viewport が正しく設定されない

**症状**
iOS Safari でページが 980px のビューポートで描画され、ズームアウト状態になる。

**原因**
Next.js 15 以降、`<meta name="viewport">` は `app/layout.tsx` から `viewport` オブジェクトとして明示的にエクスポートする必要がある。設定がないとブラウザのデフォルト（980px）が使われることがある。
