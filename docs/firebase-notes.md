...
**解決策**
`FirestoreDataConverter` で取得時に ISO 文字列に変換し、UI 側では `.toDate()` を呼ばない設計にする。
...
// 使用例
const q = query(collection(db, 'tasks').withConverter(taskConverter), where('userId', '==', userId));
```

---

### Firestore に `createdAt` をクライアント時刻で書き込むと時刻ズレが起きる

**症状**
ドキュメントの `createdAt` がユーザーのデバイス時刻になり、端末間でソート順がずれる。

**原因**
`new Date()` はクライアント時刻のため、端末の時計がずれていると不整合が起きる。

**解決策**
日時フィールドは `serverTimestamp()` で書き込む。

```typescript
import { addDoc, serverTimestamp } from 'firebase/firestore';

await addDoc(collection(db, 'tasks').withConverter(taskConverter), {
    ...data,
    userId,
...
### `onSnapshot` のクリーンアップを忘れてメモリリークが発生する

**症状**
コンポーネントをアンマウントしてもリスナーが残り、メモリリークや意図しない state 更新が発生する。

**原因**
`useEffect` 内で `onSnapshot` を呼んだが、返却された `unsubscribe` 関数を `return` し忘れている。

**解決策**
`onSnapshot` の戻り値（unsubscribe 関数）を必ず `return` する。

```typescript
// NG: return がないのでアンマウント後もリスナーが残る
useEffect(() => {
    onSnapshot(q, handler);
}, []);

// OK: return でクリーンアップ関数を登録する
useEffect(() => {
...
**解決策 A**: `initializeFirestore` の `ignoreUndefinedProperties` オプションを有効にする。
...
注意: `ignoreUndefinedProperties: true` は便利だが、`undefined` フィールドが意図せずスキップされてバリデーション漏れになるリスクがある。型レベルで `undefined` を排除できるなら解決策 B の方が安全。

**解決策 B（推奨）**: 書き込み前に `undefined` キーを除去するヘルパーを使う。
...
API キーが露出した際の不正アクセス防止として App Check の導入を検討する。reCAPTCHA v3 と組み合わせると、ブラウザからの正規リクエストのみを許可できる。
...
設定後は Firebase Console → App Check → 各サービスのエンフォースメントを有効にする手順が必要（`initializeAppCheck` だけでは強制されない）。

Firestore のセキュリティルールはレート制限をかけられないため、高頻度の書き込みへの防御が必要な場合は App Check または Cloud Functions でのバリデーションを組み合わせる。
...
export function LocalTasksProvider({ children }: { children: React.ReactNode }) {
    ...
    return (
        <TasksContext.Provider value={{ tasks, loading: false, error: null, addTask, updateTask, deleteTask }}>
            {children}
...
注意: `NEXT_PUBLIC_` 接頭辞はクライアントバンドルに公開される。`.env.production` で明示的に空にしておかないと、本番ビルドにローカルデータが混入するリスクがある。

```bash
# .env.production
NEXT_PUBLIC_DATA_SOURCE=   # 必ず空にして Firebase を使う
```
